...
void Player::update(float deltaTime)
{
	...
	std::cout << "PLAYER: " << std::to_string(std::round(mpTransform->getPosition().getY())) << std::endl;

	mpBounds->updateBounds(static_cast<int>(std::round(mpTransform->getPosition().getX())),
		static_cast<int>(std::round(mpTransform->getPosition().getY())),//when this is neg the bottom works, pos the top works...
